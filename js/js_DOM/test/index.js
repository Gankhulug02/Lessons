...
let a = false
  //жишээ хэрэглээ
  for(let i=0; i<inputs.email.value.length; i++){
    if(inputs.email.value[i] == "@"){
        if(inputs.psw.value.length >=8){
          if(inputs.psw.value !== inputs.repeatPsw.value){
            a = false;
          }else if(inputs.psw.value == inputs.repeatPsw.value){
            a = true;
        }
    }
  }
